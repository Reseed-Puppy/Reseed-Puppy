...
# 辅种主函数
def qbseed(qb):
    skiphash = "true" if qb["skiphash"] == 2 else "false"
    isaction = "false" if qb["isaction"] == 2 else "true"
    ...
    if qb["url"] == None or qb["url"] == "":
        ...
        for site in filter(lambda x: x["passkey"], sites):
            pieces_hash_groups = [
                pieces_hash_list[i : i + 100]
                for i in range(0, len(pieces_hash_list), 100)
            ]
            for group_list in pieces_hash_groups:
                ...
                data = {"passkey": site["passkey"], "pieces_hash": group_list}
                url = site["apiUrl"]
                try:
                ...
                if isinstance(response_json.get("data"), dict):
                    for value in response_json["data"]:
                        ...
                        torrent_info = qbt_client.torrents_info(
                            torrent_hashes=info_hash_topieces[value]
                        )
                        # 判断种子是否已经在下载器中
                        if torrent_info:
                            # 获取种子的保存路径和状态
                            ...
                            if state != "downloading":
                                if (
                                    qbt_client.torrents_add(
                                        urls=f"{site['siteUrl']}download.php?id={response_json['data'][value]}&passkey={site['passkey']}",
                                        save_path=save_path,
                                        is_skip_checking=skiphash,
                                        is_paused=isaction,
                                    )
                                    == "Ok."
                                ):
                                    ...
                                    logger.info(
                                        "%s ：正在添加到下载器中", torrent_name_topieces[value]
                                    )
